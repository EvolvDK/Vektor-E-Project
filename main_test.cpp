...
int main()
{
	// Fetch simulation data (acc and dir)
	vector<float> throttle;
	vector<float> direction;

	ifstream file;
	file.open("data.txt");
	file.ignore(); // Advance one char to avoid first "["

	float value;
	while (file >> value) // Extract throttle values
	{
		throttle.push_back(value);

		if (file.peek() == ',')
			file.ignore(2); // Advance two char to avoid ", "

		if (file.peek() == ']') 
		{ 
			file.ignore(4); // Advance two char to avoid "]\n["
			break;
		}
	}
	...
	cout << "Simulation data loaded." << endl;

	//for (float i : throttle) cout << i << ", ";
	//cout << endl;

	//for (float i : direction) cout << i << ", ";
	//cout << endl;

	cout << "size: " << throttle.size() << ", " << direction.size() << endl;

	Uart u;

  	unsigned char m[3];
  	m[0]= (unsigned char) ((throttle[0] * 100 / 8) + 100);
  	m[1]= (unsigned char) (direction[0] + 16);
  	m[2]= (unsigned char) '\0';

	// Timer
	chrono::duration<double> period(0.088); // s
	...
	while (i < throttle.size())
	{
		elapsed = chrono::duration_cast<chrono::duration<double>>(chrono::high_resolution_clock::now() - start);
		//cout << elapsed.count() << " - " << period.count() << endl;

		if (elapsed > period)
		{
			elapsed = chrono::duration<double>(0);
			start = chrono::high_resolution_clock::now();
			i++;


			// Send uart
			m[0]= (unsigned char) (throttle[i] * 100 / 8)+100;
  		m[1]= (unsigned char) (direction[i]+16);
  		m[2]= (unsigned char) '\0';

			u.sendUart(m);
			//cout << "Sent " << i << " : " << (signed char)(throttle[i]) * 100 / 8 << ", " << direction[i] << endl;
      cout << "Sent " << i << " : " << (signed char)(throttle[i]) * 100 / 8 << " / " << (char) m[0] << ", " << direction[i] << " / " << static_cast<char>(m[1]) << endl;
