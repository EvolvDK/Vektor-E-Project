...
Uart::Uart(SharedDataBuffer<LawCommand>& commandBuffer)
        : commandBuffer_(commandBuffer), isOngoing_(false) {
    int ii, jj, kk;
...
void Uart::startSender() {
    if (!isOngoing_) {
        isOngoing_ = true;
        commandBuffer_.resetInterrupt();
        senderThread_ = std::thread(&Uart::run, this);
    }
}

void Uart::stopSender() {
    if (isOngoing_) {
        closeUart();
        isOngoing_ = false;
        commandBuffer_.interruptWait();
...
void Uart::sendCommand(const LawCommand& command){
...
void Uart::run(){
    ...
    while (isOngoing_.load()) {
        ...
        if (currentTime >= nextSendTime) {
            commandBuffer_.waitForData();
            auto command = commandBuffer_.getData();
